...
def get_label_list(raw_dataset, split="train") -> list[str]:
    """Get the list of labels from a classification dataset split."""
...
def main():
    ...
    if predict_only:
        ...
        # Cast float labels to int for classification (e.g., 1.0-5.0 -> 1-5)
        if getattr(_label_feature, "dtype", None) in ["float32", "float64"]:
            logger.info("Label dtype is float, casting to int32 for multiclass classification.")
        ...
        # Build label list from union of train + all val/test splits (avoid unseen labels)
    ...
    logger.info(f"setting problem type to single_label_classification (multiclass, {num_labels} classes)")
    ...
    # cache 路径一致。preprocess_function 是扁平 closure，pickle 跨 rank 确定，
    # HF 默认机制即可工作。
    ...
    if training_args.metric_for_best_model is not None:
        best = training_args.metric_for_best_model
        # 合法格式：
        #   单验证集：  eval_accuracy / eval_auc
        #   多验证集：  eval_validation_0_accuracy / eval_validation_1_auc
        valid = (
            best == "eval_accuracy" or best.endswith("_accuracy")
            or best == "eval_auc" or best.endswith("_auc")
        )
        if not valid:
            raise ValueError(
                f"metric_for_best_model='{best}' 不合法。"
                f"多分类脚本只支持 eval_accuracy 或 eval_auc（含多验证集前缀 eval_validation_N_*）。"
            )
    ...
    if training_args.do_predict:
        ...
        for split_name, predict_dataset in predict_datasets.items():
            ...
            if raw_predictions.shape[1] == 2:
                ...
                # 多分类：输出 prediction_label（argmax 类别名）+ prediction_prob（对应概率）
                probs = scipy.special.softmax(raw_predictions, axis=1)
                pred_indices = np.argmax(probs, axis=1)
