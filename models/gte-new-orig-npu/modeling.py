...
class NewAttention(nn.Module):
    """NPU 原生 attention：基于 torch_npu.npu_fusion_attention。

    两条路径：
    - length 不为 None（unpad 模式）：走 actual_seq_qlen/kvlen 变长 API，
      等价于 xformers 的 BlockDiagonalMask，padding 计算完全消除。
    - length 为 None（padded 模式）：走标准 atten_mask 路径。
    """
    def __init__(self, config: NewConfig):
        super().__init__()
        self.config = config
        ...
        # Q/K/V 三投影（fusion_attention 不接受 packed qkv）
    ...
    def forward(
        ...
        if output_attentions:
            raise RuntimeError("npu_fusion_attention does not output attention weights")

        # Q/K/V 投影：[B, S, n_head, head_dim]
        if qkv_inputs is None:
            qkv_inputs = (hidden_states, hidden_states, hidden_states)
        shape_hd = (self.num_attention_heads, self.attention_head_size)
        query_states = self.q_proj(qkv_inputs[0]).view(qkv_inputs[0].shape[:-1] + shape_hd)
        key_states = self.k_proj(qkv_inputs[1]).view(qkv_inputs[1].shape[:-1] + shape_hd)
        value_states = self.v_proj(qkv_inputs[2]).view(qkv_inputs[2].shape[:-1] + shape_hd)

        # RoPE
        if self.config.position_embedding_type == 'rope':
            query_states, key_states = apply_rotary_pos_emb(query_states, key_states, *rope_embeds)

        # fusion_attention 的 keep_prob 与 nn.Dropout.p 语义相反（keep = 1 - drop）
        keep_prob = 1.0 - self.dropout.p if self.training else 1.0

        if length is not None:
            # unpad + 变长模式：actual_seq_qlen/kvlen 告诉 kernel batch 边界
            context_layer, _, _, _ = torch_npu.npu_fusion_attention(
                query_states, key_states, value_states,
                head_num=self.num_attention_heads,
                input_layout="BSND",
                actual_seq_qlen=length,
                actual_seq_kvlen=length,
                keep_prob=keep_prob,
            )
        else:
            # padded 模式：标准 extended mask
            context_layer, _, _, _ = torch_npu.npu_fusion_attention(
                query_states, key_states, value_states,
                head_num=self.num_attention_heads,
                input_layout="BSND",
                atten_mask=attention_bias,
                keep_prob=keep_prob,
            )

        # [B, S, n_head, head_dim] → [B, S, all_head_size]
...
class NewModel(NewPreTrainedModel):
    ...
    def forward(
        ...
        token_type_ids: Optional[torch.Tensor] = None,
        position_ids: Optional[torch.Tensor] = None,
        head_mask: Optional[torch.Tensor] = None,  # kept for API compat; fusion_attention ignores
        inputs_embeds: Optional[torch.Tensor] = None,
        ...
        # NPU fusion_attention 通过 actual_seq_qlen/kvlen 自己识别 batch 边界，
        # unpad 路径无需 attention_bias；padded 路径用标准 extended mask
        if length is not None:
            attention_bias = None
        else:
...
class NewForMaskedLM(NewPreTrainedModel):
    ...
    def forward(
        ...
        # === EVAL FIX: 适配 run_mlm.py 的 Trainer.evaluate ===
        # 官方原版在 unpad_inputs=True 时，eval 也走 subset_indices，
        # 返回 logits 形状 [1, num_masked, vocab]，与 Trainer 期望的 [batch, seq_len, vocab] 不匹配。
        # 修复：eval 时不 subset_indices（让 sequence_output 含全部 valid_tokens），
        #       forward 末尾 re-pad prediction_scores 回 [batch, seq_len, vocab]。
        # train 路径保持原 unpad + subset 优化，速度不受影响。
        ...
        if labels is not None:
            # === EVAL FIX: 用独立变量算 loss，避免覆盖 prediction_scores ===
            if subset_indices is not None:
                # train + unpad + subset：labels 已压缩到 [1, valid_tokens]
        ...
        # === EVAL FIX: unpad 路径下，eval 时 re-pad logits 对齐 padded labels ===
