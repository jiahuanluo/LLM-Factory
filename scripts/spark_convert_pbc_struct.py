"""Spark 版 pbc_struct 转换器：原始 CrisPbc JSON 报文（大表）→ pbc_struct 特征表。

适用：原始报文量大无法导出离线，在 Spark 集群内用 UDF 转换。
...
与 scripts/convert_mock_to_pbc_struct.py 语义完全一致：
  - user 32 numeric = 13 基础（无 score）+ 19 report 级聚合
  - 6 类账户 13 numeric + 13 cat + 60 月 paystate（PAYSTATE_VOCAB 编码）
  - 金额/大计数 log1p、年份 clamp、credit_utilization clamp [0,1.5]

vocab 两遍扫描：pass1 在集群内 distinct 收集码值 → driver 建 vocab（0=UNK，1..N 字典序，
与 SQL 04_build_cat_vocab / 离线转换器约定一致）→ broadcast → pass2 转换，保证 UNK=0。

⚠️ 常量与 src/pbc_credit/fields.py 同步（自包含是为了 executor 不依赖 src 包）——
   改 fields.py 维度后必须同步本文件。

用法（集群/notebook，spark 已就绪）：
  直接粘贴整份文件，改 RUN_DATE 后执行 run_spark()
本地自检（无 pyspark 依赖，只测纯函数）：
...
# (字段, 码值表名)：列顺序 = cat_ids/cat_mask 数组顺序
USER_CAT_FIELDS = [
...
# account numeric 13 列：做 log1p 的下标（aj01,aj02,aj03,bj01,bj02,cj02,cj06,st_amt）
LOG1P_COLS = {0, 1, 2, 4, 5, 6, 7, 12}

# ============================================================
# 纯函数（与 convert_mock_to_pbc_struct.py 语义一致，可本地单测）
# ============================================================
...
def collect_vocab_values(r: dict) -> list:
    """收集一条报文里的非空码值，返回 ['section|table|value', ...]（供 Spark distinct）。"""
    vals = []
    person = r.get('personInfo') or {}
    sources = {
        **{k: person.get('identity') or {} for k in ('pb01ad01', 'pb01ad02', 'pb01ad03', 'pb01ad04', 'pb01ad05')},
...
def build_vocab_from_pairs(pairs: list) -> dict:
    """['section|table|value', ...] → {'user': {table: {'<UNK>':0, val:id}}}, id 按字典序 1..N。"""
    sets = {'user': {}, 'account': {}}
    for p in pairs:
...
def parse_report_to_struct_json(report_json_str: str, vocab: dict) -> str:
    ...
    try:
        ...
        return json.dumps({'_error': f'{type(e).__name__}: {e}'[:200], 'report_id': rid}, ensure_ascii=False)


# ============================================================
# Spark driver（在集群上执行；本地自检走 --local-test）
# ============================================================
...
def run_spark():
    ...
    n_tables = sum(len(vocab[s]) for s in vocab)
    print(f'=== pass1 vocab: user {len(vocab["user"])} 表 + account {len(vocab["account"])} 表'
          f'（{sum(len(t) - 1 for s in vocab.values() for t in s.values()):,} 个码值）===')
    try:  # vocab 落盘（driver 本地）：混训 mock 时与 cat_vocab_mock.json 取并集后两侧重编码
...
def local_test(path: str) -> int:
    ...
    n_user, n_acc = len(out['user_numeric']), len(out.get('d1_numeric') or [])
    print(f'user_numeric: {n_user} 维（预期 32）')
    for t in [a.lower() for a in ACCOUNT_TYPES]:
