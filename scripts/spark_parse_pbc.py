"""Spark PBC 处理作业 — 独立模块，vocab 已预生成。

============================================================
  生产用法（PySpark DataFrame API）
============================================================
...
============================================================
  vocab 文件分发
============================================================
spark-submit --files /hdfs/path/cat_vocab.json your_job.py

executor 启动时 cwd 会自动有 cat_vocab.json；模块首次调用时 lazy load。
若文件名/路径不同，设环境变量 PBC_VOCAB_PATH=/path/to/vocab.json。

也可在 driver 端显式调用 init_vocab(path) 预加载（可验证路径有效性）。
"""
...
# ============================================================
# Vocab 全局（executor 端 lazy load）
# ============================================================

_VOCAB: dict | None = None
_DEFAULT_VOCAB_FILENAME = 'cat_vocab.json'


def init_vocab(path_or_dict) -> None:
    """显式初始化 vocab（可选）。driver 端调用可提前验证路径。"""
    global _VOCAB
    if isinstance(path_or_dict, str):
        ...
        raise TypeError(f'vocab must be path str or dict, got {type(path_or_dict)}')


def _get_vocab() -> dict:
    """lazy load vocab（首次调用时）。executor 端用。"""
    global _VOCAB
    if _VOCAB is not None:
        return _VOCAB
    path = os.environ.get('PBC_VOCAB_PATH', _DEFAULT_VOCAB_FILENAME)
...
# ============================================================
# 字段定义（与 src/pbc_credit/fields.py 同步）
# ============================================================
...
def build_user(report: dict, ref: datetime.datetime, vocab: dict) -> dict:
    ...
    cat_ids = [
        ...
        for v, (_p, table) in zip(cat_values, USER_CAT_FIELDS)
    ]
    return {'numeric': numeric, 'cat_ids': cat_ids, 'cat_mask': cat_mask}


def build_summary(report: dict, vocab: dict) -> dict:
    sinfo = get_path(report, 'summaryInfo', {}) or {}
    nums, cats, cmask, cat_ids = [], [], [], []
    ...
    for name, is_list, num_fields, cat_fields in SUMMARY_TABLES:
        ...
        if node is None:
            for _ in num_fields:
                nums.append(0.0)
            for _f, t in cat_fields:
                cats.append(None); cmask.append(0)
                cat_ids.append(0 if not t else 0)
            continue
        ...
        if is_list:
            ...
            for cf, t in cat_fields:
                v = items[0].get(cf) if items else None
                cats.append(v)
                cmask.append(1 if items else 0)
                cat_ids.append(encode_value('summary', t or '', v, vocab) if t else 0)
        else:
            for nf in num_fields:
    ...
    return {'numeric': nums, 'cat_ids': cat_ids, 'cat_mask': cmask}
...
def build_accounts(report: dict, vocab: dict) -> dict:
    ...
    for t, items in by_type.items():
        k = t.lower()
        numeric, cat_ids, cat_mask, paystate = [], [], [], []
        for a in items:
            basic = a.get('accountBasic', {}) or {}
            ...
            row_cat, row_cmask = [], []
            row_ids = []
            for f, tbl in ACCOUNT_CAT_FIELDS:
                v = basic.get(f)
                row_cat.append(v)
                row_cmask.append(0 if v in (None, '') else 1)
                row_ids.append(encode_value('account', tbl or '', v, vocab) if tbl else 0)
            cat_ids.append(row_ids)
...
def build_queries(report: dict, ref: datetime.datetime, vocab: dict) -> dict:
    recs = get_path(report, 'queryRecords', []) or []
    numeric, cat_ids, cat_mask = [], [], []
    for r in recs:
        ...
        row_ids = []
        row_cmask = []
        for f, tbl in QUERY_CAT_FIELDS:
            v = r.get(f)
            row_ids.append(encode_value('query', tbl or '', v, vocab) if tbl else 0)
            row_cmask.append(0 if v in (None, '') else 1)
        cat_ids.append(row_ids)
        cat_mask.append(row_cmask)
    return {'numeric': numeric, 'cat_ids': cat_ids, 'cat_mask': cat_mask}
...
def build_publics(report: dict, ref: datetime.datetime) -> dict:
    ...
    for _node, type_name in PUBLIC_TYPES:
        ...
        for it in items:
            ...
            cmask.append([1])
    return {'numeric': numeric, 'cat_ids': cat_ids, 'cat_mask': cmask}


# ============================================================
# 主入口：JSON → sample dict（已编码，纯 list）
# ============================================================

def build_sample(report: dict, vocab: dict) -> dict:
    """把 CrisPbc.json 解析为 sample dict（纯 list，可直接 json.dumps）。"""
    ref = _report_ref_date(report)
    sample = {}

    u = build_user(report, ref, vocab)
    sample['user_numeric'] = u['numeric']
    sample['user_cat_ids'] = u['cat_ids']
    sample['user_cat_mask'] = u['cat_mask']

    s = build_summary(report, vocab)
    sample['summary_numeric'] = s['numeric']
    sample['summary_cat_ids'] = s['cat_ids']
    sample['summary_cat_mask'] = s['cat_mask']

    accounts = build_accounts(report, vocab)
    for k, a in accounts.items():
        sample[f'{k}_numeric'] = a['numeric']
        sample[f'{k}_cat_ids'] = a['cat_ids']
        sample[f'{k}_cat_mask'] = a['cat_mask']
        sample[f'{k}_paystate'] = a['paystate']
        sample[f'{k}_mask'] = a['mask']

    q = build_queries(report, ref, vocab)
    sample['query_numeric'] = q['numeric']
    sample['query_cat_ids'] = q['cat_ids']
    ...
    sample['public_cat_ids'] = p['cat_ids']
...
def parse_report_to_struct_json(report_json_str: str) -> str:
    """Spark UDF 主函数：JSON 字符串 → pbc_struct JSON 字符串。

    单参数版（vocab 从模块全局 lazy load，匹配生产 `udf(parse_report_to_struct_json, StringType())`）。
